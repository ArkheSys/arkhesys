...
namespace Modelo
{
    public partial class Banco: IMetodosCwork
    {
        ...
        public IQueryable getLista(DataClassesDataContext db, String pFiltro)
        {
            IQueryable lista;
            lista = from banco in db.Bancos
                    where banco.Codigo.ToString() == pFiltro ||
                    banco.Nome.Contains(pFiltro)
                    select new
                    {
                        Codigo = banco.Codigo,
                        Nome = banco.Nome,
        ...
        {
            return (from banco in db.Bancos
                    select new
                    {
                        Nome = banco.Nome + " | " + banco.Codigo,
                        ID = banco.ID
                    }).ToList();
        }
        ...
        public int getID(DataClassesDataContext db, string pValor)
        {
            if (pValor.Length != 0)
            {
                return (from u in db.Bancos where u.Codigo.ToString() == pValor select u).Single().ID;
            }
            else
            {
                return 0;
        ...
        public Int32 RetornaMAXCodigo(DataClassesDataContext db)
        {
            ...
            try
            {
                maxCodigo = (from b in db.Bancos
                             where b.Codigo == db.Bancos.Max(c => c.Codigo)
