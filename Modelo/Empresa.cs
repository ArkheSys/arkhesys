...
namespace Modelo
{
    public partial class Empresa: Modelo.IMetodosCwork
    {
        /// <summary>
        ...
        {
            IQueryable lista;
            lista = from empresa in db.Empresas
                    select new
                    {
        ...
        public IQueryable getLista(DataClassesDataContext db, String pFiltro)
        {
            IQueryable lista;
            lista = from empresa in db.Empresas
                    where empresa.Codigo.ToString() == pFiltro ||
                    empresa.Nome.Contains(pFiltro)
                    select new
                    {
                        Codigo = empresa.Codigo,
                        Nome = empresa.Nome,
        ...
        public IList getListaCombo(DataClassesDataContext db)
        {
            return (from empresa in db.Empresas
                    select new
                    {
                        Nome = empresa.Nome + " | " + empresa.Codigo,
                        ID = empresa.ID
                    }).ToList();
        }
        ...
        {
            return (from u in db.Empresas where u.ID == pID select u).Single();
        }
        ...
        public int getID(DataClassesDataContext db, string pValor)
        {
            if (pValor.Length != 0)
            {
                return (from u in db.Empresas where u.Codigo.ToString() == pValor select u).Single().ID;
            }
            else
            {
                return 0;
        ...
        public Int32 RetornaMAXCodigo(DataClassesDataContext db)
        {
            ...
            try
            {
                maxCodigo = (from e in db.Empresas
                             where e.Codigo == db.Empresas.Max(b => b.Codigo)
