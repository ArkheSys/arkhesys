...
namespace Modelo
{
    public partial class TipoDocumento : Modelo.IMetodosCwork
    {
        ...
        {
            IQueryable lista;
            lista = from t in db.TipoDocumentos
                    select new
                    {
                        Codigo = t.Codigo,
                        Nome = t.Nome,
        ...
        public IQueryable getLista(DataClassesDataContext db, String pFiltro)
        {
            ...
            lista = from t in db.TipoDocumentos
                    ...
                    t.DescReduzida.Contains(pFiltro)
                    select new
                    {
                        Codigo = t.Codigo,
                        Nome = t.Nome,
        ...
        {
            return (from t in db.TipoDocumentos
                    select new
                    {
                        Nome = t.Nome,
                        ID = t.ID
        ...
        {
            return (from u in db.TipoDocumentos where u.ID == pID select u).Single();
        }
        ...
        public int getID(DataClassesDataContext db, string pValor)
        {
            if (pValor.Length != 0)
            {
                return (from u in db.TipoDocumentos where u.Codigo.ToString() == pValor select u).Single().ID;
            }
            else
            {
                return 0;
        ...
        public Int32 RetornaMAXCodigo(DataClassesDataContext db)
        {
            ...
            try
            {
                maxCodigo = (from t in db.TipoDocumentos
                             where t.Codigo == db.TipoDocumentos.Max(b => b.Codigo)
