...
namespace Modelo
{
    public partial class Classificacao: IMetodosCwork
    {
        ...
        {
            IQueryable lista;
            lista = from c in db.Classificacaos
                    select new
                    {
                        Codigo = c.Codigo,
        ...
        {
            return (from c in db.Classificacaos
                    select new
                    {
                        Nome = c.Descricao + " | " + c.Codigo,
                        ID = c.ID
                    }).ToList();
        }
        ...
        {
            return (from u in db.Classificacaos where u.ID == pID select u).Single();
        }
        ...
        public int getID(DataClassesDataContext db, string pValor)
        {
            if (pValor.Length != 0)
            {
                return (from u in db.Classificacaos where u.Codigo.ToString() == pValor select u).Single().ID;
            }
            else
            {
                return 0;
        ...
        public Int32 RetornaMAXCodigo(DataClassesDataContext db)
        {
            ...
            try
            {
                maxCodigo = (from c in db.Classificacaos
                             where c.Codigo == db.Classificacaos.Max(b => b.Codigo)
