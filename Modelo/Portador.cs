...
namespace Modelo
{
    public partial class Portador : Modelo.IMetodosCwork
    {
        ...
        {
            IQueryable lista;
            lista = from portador in db.Portadors
                    select new
                    {
                        Codigo = portador.Codigo,
                        Nome = portador.Nome,
                        ID = portador.ID
                    };
            return lista;
        ...
        public IQueryable getLista(DataClassesDataContext db, String pFiltro)
        {
            IQueryable lista;
            lista = from portador in db.Portadors
                    where portador.Codigo.ToString() == pFiltro ||
                    portador.Nome.Contains(pFiltro)
                    select new
                    {
                        Codigo = portador.Codigo,
                        Nome = portador.Nome,
        ...
        {
            return (from portador in db.Portadors
                    select new
                    {
                        Nome = portador.Nome + " | " + portador.Codigo,
                        ID = portador.ID
                    }).ToList();
        }
        ...
        {
            return (from u in db.Portadors where u.ID == pID select u).Single();
        }
        ...
        public int getID(DataClassesDataContext db, string pValor)
        {
            if (pValor.Length != 0)
            {
                return (from u in db.Portadors where u.Codigo.ToString() == pValor select u).Single().ID;
            }
            else
            {
                return 0;
        ...
        public Int32 RetornaMAXCodigo(DataClassesDataContext db)
        {
            ...
            try
            {
                maxCodigo = (from p in db.Portadors
                             where p.Codigo == db.Portadors.Max(b => b.Codigo)
