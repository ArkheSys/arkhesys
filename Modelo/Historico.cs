...
namespace Modelo
{
    public partial class Historico : Modelo.IMetodosCwork
    {
        /// <summary>
        ...
        public IQueryable getLista(DataClassesDataContext db, String pFiltro)
        {
            IQueryable lista;
            lista = from h in db.Historicos
                    where h.Codigo.ToString() == pFiltro ||
                    h.Nome.Contains(pFiltro)
                    select new
                    {
                        Codigo = h.Codigo,
                        Nome = h.Nome,
        ...
        {
            return (from h in db.Historicos
                    select new
                    {
                        Nome = h.Nome,
                        ID = h.ID
        ...
        {
            return (from u in db.Historicos where u.ID == pID select u).Single();
        }
        ...
        public int getID(DataClassesDataContext db, string pValor)
        {
            if (pValor.Length != 0)
            {
                return (from u in db.Historicos where u.Codigo.ToString() == pValor select u).Single().ID;
            }
            else
            {
                return 0;
        ...
        public Int32 RetornaMAXCodigo(DataClassesDataContext db)
        {
            ...
            try
            {
                maxCodigo = (from a in db.Historicos
                             where a.Codigo == db.Historicos.Max(b => b.Codigo)
