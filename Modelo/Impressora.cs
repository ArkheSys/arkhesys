...
namespace Modelo
{
    public partial class Impressora : Modelo.IMetodosCwork
    {
        /// <summary>
        ...
        {
            IQueryable lista;
            lista = from imp in db.Impressoras
                    select new
                    {
                        Codigo = imp.Codigo,
                        Nome = imp.Nome,
        ...
        public IQueryable getLista(DataClassesDataContext db, String pFiltro)
        {
            IQueryable lista;
            lista = from imp in db.Impressoras
                    where imp.Codigo.ToString() == pFiltro ||
                    imp.Nome.Contains(pFiltro)
                    select new
                    {
                        Codigo = imp.Codigo,
                        Nome = imp.Nome,
        ...
        {
            return (from imp in db.Impressoras
                    select new
                    {
                        Nome = imp.Nome + " | " + imp.Codigo,
                        ID = imp.ID
                    }).ToList();
        }
        ...
        {
            return (from u in db.Impressoras where u.ID == pID select u).Single();
        }
        ...
        public int getID(DataClassesDataContext db, string pValor)
        {
            if (pValor.Length != 0)
            {
                return (from u in db.Impressoras where u.Codigo.ToString() == pValor select u).Single().ID;
            }
            else
            {
                return 0;
        ...
        public Int32 RetornaMAXCodigo(DataClassesDataContext db)
        {
            ...
            try
            {
                maxCodigo = (int)(from e in db.Impressoras
                             where e.Codigo == db.Impressoras.Max(b => b.Codigo)
                             select e).Single().Codigo;
