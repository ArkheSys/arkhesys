...
namespace Modelo
{
    public partial class Produto_Icm : IMetodosCwork
    {
        /// <summary>
        ...
        public IQueryable getLista(DataClassesDataContext db)
        {
            ...
            lista = from u in db.Produto_Icms
                    select new
                    {
        ...
        {
            return (from u in db.Produto_Icms
                    select new
                    {
        ...
        {
            return (from u in db.Produto_Icms where u.ID == pID select u).Single();
        }
        ...
        public int getID(DataClassesDataContext db, string pValor)
        {
            if (pValor.Length != 0)
            {
                try
                {
                    return (from o in db.Produto_Icms where o.ID.ToString() == pValor select o).Single().ID;
                }
                catch (Exception)
                {
                    return 0;
        ...
        public int RetornaMAXCodigo(DataClassesDataContext db)
        {
            ...
            try
            {
                maxCodigo = db.Produto_Icms.Max(m => m.Codigo).Value;
