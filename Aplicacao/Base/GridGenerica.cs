...
namespace Aplicacao
{
    public partial class GridGenerica<T> : Form, IFormGrid where T : ModeloBase, new()
    {
        ...
        DataTable tipoNotaTable;
        private object[] _ChildFormParameters;
        bool tiponota;
        public bool FocoNaGrid = false;
        ...
        public IList<TupleIFormGrid> Grids
        {
            get
            {
                if (grids == null)
                {
                    ...
                    foreach (TupleIFormGrid item in grids)
                    {
                        string tipo = item.Tipo.Name.ToString();
                        if (tipo == "TipoNota")
                        {
                            tiponota = true;
                        }
        ...
        public GridGenerica(IList<T> list, T selecionado, IFormManut<T> formManut, bool adicionaFiltro)
            : this(list, formManut, selecionado, adicionaFiltro)
        {
        }
        ...
        private GridGenerica(IList<T> list, T selecionado, Func<IList<T>> atualizador, bool adicionaFiltro, DateTime? inicioFiltro, DateTime? fimFiltro, Dictionary<string, object> parametros)
        {
            ...
            // AJUSTE DATAS
            inicioFiltro = inicioFiltro?.Date;
            fimFiltro = fimFiltro?.Date.AddHours(23).AddMinutes(59).AddSeconds(59);

            if (adicionaFiltro)
            {
                if (inicioFiltro.HasValue && fimFiltro.HasValue)
                {
                    ucFiltroGridGenerica.deDataInicial.DateTime = inicioFiltro.Value;
                    ucFiltroGridGenerica.deDataFinal.DateTime = fimFiltro.Value;
                }
...
#pragma warning disable CS0693 // Parâmetro de tipo "T" tem o mesmo nome do parâmetro de tipo de tipo externo "GridGenerica<T>"
        private IFormManut<T> CriaFormFilho<T>(Type tipo, params object[] parametros)
#pragma warning restore CS0693 // Parâmetro de tipo "T" tem o mesmo nome do parâmetro de tipo de tipo externo "GridGenerica<T>"
        {
            try
            {
        ...
        protected virtual void DeletarLayout()
        {
            ...
            {
                MessageBox.Show("Layout restaurado. Feche a abra o formulário novamente.", "CWork Gestão", MessageBoxButtons.OK, MessageBoxIcon.Information);
                _salvar = false;
        ...
        {
            base.OnShown(e);
            foreach (GridColumn item in gvPrincipal.Columns)
            {
                if (item.ColumnType == typeof(DateTime))
                    item.FilterMode = ColumnFilterMode.Value;
                else
                {
                    if (this is GridProdutoGen)
                    {
                        item.FilterMode = DevExpress.XtraGrid.ColumnFilterMode.DisplayText;
                        item.OptionsFilter.AutoFilterCondition = item.ColumnType == typeof(Decimal) ? AutoFilterCondition.Contains : AutoFilterCondition.BeginsWith;
                    }
                    else
                    {
                        item.FilterMode = DevExpress.XtraGrid.ColumnFilterMode.DisplayText;
                    ...
                    if (item.ColumnType == typeof(String))
                    {
                        if (config == null)
                            config = ConfiguracaoController.Instancia.GetConfiguracao();

                        if (config.BuscarProdutoPesquisaDinamica == 1)
                ...
                {
                    item.SummaryItem.DisplayFormat = "Total = {0:C2}";
                    item.SummaryItem.FieldName = item.FieldName;
                    item.SummaryItem.SummaryType = SummaryItemType.Sum;
                }
            }

            if (FocoNaGrid)
                ActiveControl = gcPrincipal;
        }
    }
}