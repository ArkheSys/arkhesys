    ...
    public partial class FormFaturamentoLoteNova : Aplicacao.IntermediariasTela.FormFaturamentoLoteIntermediaria
    {
        ...
        public FormFaturamentoLoteNova(Pedido pedido)
        {
                ...
                "Cwork Gestão", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes))
            {
                ...
                foreach (Pedido pedidosAberto in pedidosAbertos)
                {
        ...
        private void SetaCamposIniciais()
        {
            ...
            {
                //throw new Exception("Houve um erro ao carregar o cliente. => "+e.Message);
                MessageBox.Show("Houve um erro ao carregar o cliente.\n" + e.Message, "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            ...
            {
                //throw new Exception("Houve um erro ao carregar a condição de pgto. => " + e.Message);
                MessageBox.Show("Houve um erro ao carregar a condição de pgto.\n" + e.Message, "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            ...
            {
                //throw new Exception("Houve um erro ao carregar a filial. => " + e.Message);
                MessageBox.Show("Houve um erro ao carregar a filial.\n" + e.Message, "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            ...
            {
                //throw new Exception("Houve um erro ao carregar o tipo do pedido. => " + e.Message);
                MessageBox.Show("Houve um erro ao carregar o tipo do pedido.\n" + e.Message, "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        ...
        private void IniciaExemplos(Pedido objPedido)
        {
            ...
            if (objPedido != null)
            {
                ...
                if (objPedido.TipoPedido.Ent_Sai == 1)
                {
                    ...
                    {
                        Ent_Sai = 1,
                        Categoria = objPedido.TipoPedido.Categoria == 0 ?
            ...
            if (ValidaFormulario() && MessageBox.Show("Deseja faturar os orçamentos/pedidos selecionados?", "Confirme", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    ...
                    if (((TipoNota)lkpTipoMovimentacao.Selecionado).ModeloDocto == TipoNota.ModeloDocumento.CupomFiscal)
                    {
                        ...
                        {
                            if (!(String.IsNullOrEmpty(p.NCM)) && ((NCMController.Instancia.LoadObjectById(p.ID_NCM) == null) || (String.IsNullOrEmpty(NCMController.Instancia.LoadObjectById(p.ID_NCM).AliqCupom))))
                            {
                    ...
                    else
                    {
                            ...
                            && (pedidos.Where(env => env.CupomEnviado != true).Count() > 0))
                        {
                            ...
                            {
                                nota = new cwkGestao.Negocio.Faturamento.Orcamento(pedidosNaoEnviados, financeiroParcelas, dtFaturamento, condicao, tipoNota, txtObservacao1.Text, txtObservacao2.Text, txtObservacao3.Text, pessoaEndereco, null, false, false, false, false, null).Nota;
                            }
                            else
                            {
        ...
        private IList<FinanceiroParcela> TransformaListaParcelas(IList<PedidoParcela> financeiroParcelas)
        {
            ...
            foreach (PedidoParcela financeiroParcela in financeiroParcelas)
            {
                ...
                parcela.FormaPagamento = (ParcelaBase.EnumFormaPagamento) Convert.ToInt32(financeiroParcela.TipoDocumento.FormaPagamento.CodigoSefaz);
