...
namespace Aplicacao
{
    public partial class FormNota : IntermediariasTela.FormManutNotaIntermediaria
    {
        ...
        protected override List<Exception> ObjetoPraTela(Control pai)
        {
            ...
            if (pai.Equals(tabDestinatario))
            {
                if (Selecionado.IDEnderecoDestinatario.HasValue)
                    AtribuiEnderecoDestinatario(PessoaEnderecoController.Instancia.LoadObjectById(Selecionado.IDEnderecoDestinatario.Value));
                else
                {
                    if (lkpPessoa.Selecionado != null)
                    {
                        if (((Pessoa)lkpPessoa.Selecionado).PessoaEnderecos.FirstOrDefault(o => o.RemetenteDestinatario) != null)
                        {
                            logicaTelaNota.SetPessoa((Pessoa)lkpPessoa.Selecionado, ((Pessoa)lkpPessoa.Selecionado).PessoaEnderecos.FirstOrDefault(o => o.RemetenteDestinatario));
                            AtribuiEnderecoDestinatario(((Pessoa)lkpPessoa.Selecionado).PessoaEnderecos.FirstOrDefault(o => o.RemetenteDestinatario));
                        }
                    }
                }
            }
        ...
        private void AtribuiExemplosLkp()
        {
            ...
            else
            {
                ...
                lkpNotaRefLista.Exemplo = new Nota { };
                lkpNotaRefLista.CamposRestricoesAND = new List<string> { };
            }
        ...
        {
            //if (Operacao == Acao.Alterar || Operacao == Acao.Consultar)
            //{
            //    Selecionado = NotaController.Instancia.LoadObjectById(Selecionado.ID);

            //    AtualizaGrid(gvNotaParcelas);
            //    //gcNotaParcelas.DataSource = Selecionado.NotaParcelas;
            ...
            {
                if (ConfiguracaoController.Instancia.GetConfiguracao().CarregarVendedor)
                    CarregarVendedor();

                txtCodigo.EditValue = NotaController.Instancia.GetNovoCodigoNota();
                ...
                //txtNumero.Text = (Selecionado.Filial.UltimoNumeroEmitidoNFe + 1).ToString();

                Selecionado.StatusNota = Nota.StatusTipo.NaoEnviada;
                Selecionado.Status = "-1";
                Selecionado.StatusLocal = null;
                Selecionado.StatusMotivo = null;
            }
            else
            {
            ...
            if (importacaoXML)
            {
                ...
                Selecionado.StatusNota = Nota.StatusTipo.Importada;
                Selecionado.Status = "11";
            }
            ...
            if (importacaoXML || (inOut == InOutType.Entrada && config.TrazerComoPadraoNoLancamentoNFe == 1) && Selecionado?.ID == 0)
            {
                if (config.TipoMovimentacaoCompras.HasValue)
                    lkpTipoNota.Localizar(config.TipoMovimentacaoCompras.Value);

                if (config.CondicaoPagamentoCompras.HasValue)
                {
                    lkpCondicao.Localizar(config.CondicaoPagamentoCompras.Value);
                ...
                if (config.TabelaPrecoCompras.HasValue)
                {
                    lkpTabelaPreco.Localizar(config.TabelaPrecoCompras.Value);
                }
            }

            else
            {
                if (config.IDTipoNotaNota.HasValue && config.IDTipoNotaNota.Value != 0)
                    lkpTipoNota.Localizar(config.IDTipoNotaNota.Value);

                if (config.IDCondicaoNota.HasValue && config.IDCondicaoNota.Value != 0)
                    lkpCondicao.Localizar(config.IDCondicaoNota.Value);
            }

            if (config.IDTabelaPrecoNota.HasValue && config.IDTabelaPrecoNota.Value!= 0)
                lkpTabelaPreco.Localizar(config.IDTabelaPrecoNota.Value);

            var pessoaFuncionario = PessoaFuncionarioController.Instancia.GetVendedorAssociadoComUsuarioLogado(Facade.getUsuarioLogado.Login);
            if (pessoaFuncionario != null)
                lkpVendedor.Localizar(pessoaFuncionario.Pessoa.ID);
        ...
        private void lkpFilial_IDChanged(object sender, EventArgs e)
        {
            ...
            //if (Operacao == Acao.Incluir)
            //lkpTipoNota.Where = " idFilial = " + Selecionado.Filial.ID.ToString();
        ...
        private void lkpPessoa_Leave(object sender, EventArgs e)
        {
            if (lkpPessoa.Selecionado != null)
            {
                if (((Pessoa)lkpPessoa.Selecionado).PessoaEnderecos.Where(o => o.RemetenteDestinatario).FirstOrDefault() == null)
                {
                    MessageBox.Show("A Pessoa selecionada não possui endereço de Destinatário", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                logicaTelaNota.SetPessoa((Pessoa)lkpPessoa.Selecionado, ((Pessoa)lkpPessoa.Selecionado).PessoaEnderecos.Where(o => o.RemetenteDestinatario).FirstOrDefault());
                AtribuiEnderecoDestinatario(((Pessoa)lkpPessoa.Selecionado).PessoaEnderecos.Where(o => o.RemetenteDestinatario).FirstOrDefault());
            }
            HabilitaDesabilitaNotaReferenciada();
            if (lkpNotaRef.Visible == true)
            {
        ...
        private void SelecionaCampos()
        {
            if (lkpTipoNota.Selecionado != null)
            {
                ...
                if (logicaTelaNota.NumeroGeradoAutomatico)
                {
                    ...
                    if (Selecionado.Numero == 1)
                    {
                        if (Selecionado.ModeloDocto == 55)
                            txtNumero.EditValue = Selecionado.Numero;

                        if (Selecionado.ModeloDocto == 65)
                            txtNumero.EditValue = Selecionado.Numero;
                    }
                    else
                        txtNumero.EditValue = Selecionado.Numero;
        ...
        private void lkpTipoNota_Leave(object sender, EventArgs e)
        {
            ...
            if (gvNotaItems.RowCount > 0 && sbGravar.Visible)
            {
                //Selecionado.TipoNota = (TipoNota)lkpTipoNota.Selecionado;
                //if (MessageBox.Show("Deseja ajustar as informações dos produtos?", "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
        ...
        {
            var config = ConfiguracaoController.Instancia.GetConfiguracao();
            tpNfRef.PageVisible = config.UsaListaReferenciaNota;

            if (config.UsaListaReferenciaNota)
                return;
        ...
        {
            var tipos = TipoNotaController.Instancia.GetByExample(lkpTipoNota.GetCriterion());
            //var filial = lkpFilial.Selecionado as Filial;
            //if (filial != null)
            //    tipos = tipos.Where(o => o.Filial.ID == filial.ID).ToList();

            GridGenerica<TipoNota> grid = new GridGenerica<TipoNota>(tipos, new FormTipoNota(), (TipoNota)lkpTipoNota.Selecionado, false);
            grid.Selecionando = true;
            if (cwkControleUsuario.Facade.ControleAcesso(grid))
                grid.ShowDialog();
            if (grid.Selecionado != null)
            ...
            if (gvNotaItems.RowCount > 0 && sbGravar.Visible)
            {
                //if (MessageBox.Show("Deseja ajustar as informações dos produtos?", "Atenção", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No) return;
                AtualizarDadosProdutos();
            }
        ...
        private void VerificarCampoExportacao()
        {
            ...
            {
                tabNFRef.TabPages.Insert(tabNFRef.TabPages.Count, tabExportacao);
            ...
            {
                tabNFRef.TabPages.Remove(tabExportacao);
                foreach (Control item in tabExportacao.Controls)
                {
                    item.Enabled = false;
                }
        ...
        {
            if (txtHoraEnvio.EditValue == null)
                txtHoraEnvio.EditValue = txtDtEnvio.DateTime;

            logicaTelaNota.SetHoraEnvio(txtHoraEnvio.EditValue == null ? txtDtEnvio.DateTime : (DateTime)txtHoraEnvio.EditValue);
        }
        ...
        private void btIncluirProduto_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if ((TipoNota)lkpTipoNota.Selecionado != null)
                        logicaTelaNota.SetTipoNota((TipoNota)lkpTipoNota.Selecionado);
        ...
        private void btAlterarServico_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (item != null)
                {
                    var formNotaItemServico = new FormNotaItemServico(item)
                    {
        ...
        private void btAlterarProduto_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (notaItemSelecionado != null)
                {
                    ...
                    {
                        if (formNotaItem.Selecionado.IDNcm != null)
                            notaItemSelecionado.IDNcm = formNotaItem.Selecionado.IDNcm;
        ...
        private void EsconderCamposProduto()
        {
            if (tcItems.TabPages.Contains(tpProdutos))
            {
                ...
                tabNFRef.TabPages.Remove(tpVolumes);
                tabNFRef.TabPages.Remove(tpICMS);
                tabNFRef.TabPages.Remove(tpImposto);
                tabNFRef.TabPages.Remove(tpTransportadora);
        ...
        private void EsconderCamposServico()
        {
            if (tcItems.TabPages.Contains(tpServicos))
            {
                ...
                tabNFRef.TabPages.Remove(tpImpostoServico);
        ...
        private void MostrarCamposProduto()
        {
            if (!tcItems.TabPages.Contains(tpProdutos))
            {
                ...
                tabNFRef.TabPages.Add(tpVolumes);
                tabNFRef.TabPages.Add(tpICMS);
                tabNFRef.TabPages.Add(tpImposto);
                tabNFRef.TabPages.Add(tpTransportadora);
        ...
        private void MostrarCamposServico()
        {
            if (!tcItems.TabPages.Contains(tpServicos))
            {
                ...
                tabNFRef.TabPages.Add(tpImpostoServico);
        ...
        {
            var TipoNota = lkpTipoNota.Selecionado as TipoNota;
            if (TipoNota == null)
                return;

            if (TipoNota.BDevolucao)
                if (TipoNota.SemPagamento)
                {
                    foreach (var selecionadoNotaParcela in Selecionado.NotaParcelas)
                    {
                        selecionadoNotaParcela.FormaPagamento = ParcelaBase.EnumFormaPagamento.SemPagamento;
                        selecionadoNotaParcela.Valor = 0;
                        selecionadoNotaParcela.BAlterado = true;
                    }
        ...
        protected override void TelaProObjeto(Control pai)
        {
            ...
            if (lkpNotaRef.Visible && lkpNotaRef.Selecionado != null)
            {
        ...
        protected override bool ValidarFormulario()
        {
            ...
            if (Selecionado.IDEnderecoDestinatario == null)
            {
                MessageBox.Show("Selecione o Endereço de Remetente/Destinatário.", "Mensagem", MessageBoxButtons.OK);
                formularioValido = false;
            }

            if (lkpPessoaTransportadora.Selecionado != null)
            {
                if (string.IsNullOrEmpty(txtTransPlaca.Text) || string.IsNullOrEmpty(txtTransPlacaUF.Text))
                {
                    MessageBox.Show("Favor informar Placa do Veículo da Transportadora e UF, ou ocorrerá um erro ao Enviar a NFe.", "Mensagem", MessageBoxButtons.OK);
                    return true;
                }
            }
            ...
            try
            {
                if (ValidarFormulario())
                {
                    if (Operacao < Acao.Consultar)
                    {
                        ...
                        if (Selecionado.TipoNota.Categoria == TipoNota.CategoriaMovimentacao.Servicos && Selecionado.Status != "2")
                        {
                        ...
                        Selecionado.PessoaInscRG = ((Pessoa)(lkpPessoa.Selecionado)).Inscricao;
                        logicaTelaNota.GetNota.PessoaInscRG = ((Pessoa)(lkpPessoa.Selecionado)).Inscricao;
                        ...
                        logicaTelaNota.Salvar(Operacao, importacaoXML);
                    ...
                    if (Operacao == Acao.Incluir)
                    {
                        if ((config.GravarEnviarNfe == 1 && Selecionado.TipoNota.ModeloDocto == TipoNota.ModeloDocumento.NFe) ||
                            (config.GravarEnviarNfce == 1 && Selecionado.TipoNota.ModeloDocto == TipoNota.ModeloDocumento.NFCe))
                        {
                            FormNFe frmNFe = new FormNFe();
                            frmNFe.IDNota = Selecionado.ID;
                            frmNFe.FillObjNFe(false);

                            if (MessageBox.Show(this, "Deseja Visualizar o Documento Fiscal?", "ATENÇÃO", MessageBoxButtons.YesNo) == DialogResult.Yes)
                                frmNFe.VisualizarPreDanfe();
                            else
                                frmNFe.EnviarNota();
                        }
                    }
        ...
        private void VerificaNota()
        {
            if (Operacao != Acao.Consultar)
            {
                if (Selecionado.BImpressa || Selecionado.Status == "2")
                {
            ...
            if (Operacao == Acao.Incluir && logicaTelaNota.Identificador != null)
            {
                if (Selecionado == null)
                    ...
                    foreach (var item in Selecionado.NotaItems)
                    {
                        ...
                        var _grupoEstoque = _prod.GrupoEstoque == null ? null : GrupoEstoqueController.Instancia.LoadObjectById(_prod.GrupoEstoque.ID);
        ...
        {
            if (Selecionado?.NotaItems != null)
                for (int i = 0; i < Selecionado.NotaItems.Count; i++)
                    Selecionado.NotaItems[i].Sequencia = i + 1;
        }

        private void lookupButton1_Click(object sender, EventArgs e)
        {
            List<PessoaEndereco> pessoaEnderecosEntrega = Selecionado.Pessoa.PessoaEnderecos.Where(i => i.RemetenteDestinatario).ToList();
            if (pessoaEnderecosEntrega.Count == 0)
                MessageBox.Show("O cliente selecionado não possui endereços de destinatário cadastrados.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Hand);
            else
            {
                GridGenerica<PessoaEndereco> gridEnderecoEntrega = new GridGenerica<PessoaEndereco>(pessoaEnderecosEntrega, new FormPessoaEndereco(), false);
                gridEnderecoEntrega.Selecionando = true;
                ...
                if (gridEnderecoEntrega.Selecionado != null)
                    AtribuiEnderecoDestinatario(gridEnderecoEntrega.Selecionado);
        ...
        {
            logicaTelaNota.AtualizarDadosTributacaoProdutos();
        ...
        private void AtribuiEnderecoDestinatario(PessoaEndereco pessoaEnderecoSelecionado)
        {
            if (pessoaEnderecoSelecionado != null)
            {
                Selecionado.IDEnderecoDestinatario = pessoaEnderecoSelecionado.ID;
                logicaTelaNota.SetPessoa((Pessoa)lkpPessoa.Selecionado, pessoaEnderecoSelecionado);

                TXT_Dest_Rua.Text = pessoaEnderecoSelecionado.Endereco;
                TXT_Dest_Bairro.Text = pessoaEnderecoSelecionado.Bairro;
                TXT_Dest_Cep.Text = pessoaEnderecoSelecionado.CEP;
                TXT_Dest_Cidade.Text = $"{pessoaEnderecoSelecionado.Cidade} | {pessoaEnderecoSelecionado.Cidade.UF.Sigla}";
                TXT_Dest_Complemento.Text = pessoaEnderecoSelecionado.Complemento;
                TXT_Dest_Numero.Text = pessoaEnderecoSelecionado.Numero;
            }
        }

        private void simpleButton1_Click(object sender, EventArgs e)
        {
            Selecionado.IDEnderecoDestinatario = null;

            TXT_Dest_Rua.Text = "";
            TXT_Dest_Bairro.Text = "";
            TXT_Dest_Cep.Text = "";
            TXT_Dest_Cidade.Text = "";
            TXT_Dest_Complemento.Text = "";
            TXT_Dest_Numero.Text = "";
        }

        private void lkbNotaRefLista_Click(object sender, EventArgs e)
        {
            InOutType inOutPesquisa = PegaInOutPesquisa();
            ...
            if (lkpTipoNota.Selecionado == null)
            {
                MessageBox.Show("Selecione o Tipo de Nota", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            if (notaSelecionada != null)
            {
                ...
                {
                    lkpNotaRefLista.Localizar(grid.Selecionado.ID);
                }
            }

            lkpNotaRefLista.Focus();
            if ((Nota)lkpNotaRefLista.Selecionado == null)
            {
                grid.Close();
                return;
            }
        ...
        private void AtualizarListaNfRef()
        {
            gcNfRef.DataSource = Selecionado.NotaReferencias;
            gcNfRef.RefreshDataSource();
            gcNfRef.Refresh();
        }

        private void simpleButton2_Click(object sender, EventArgs e)
        {
            var nota = (Nota)lkpNotaRefLista.Selecionado;

            if(nota == null)
            {
                MessageBox.Show("Selecione uma nota!", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (string.IsNullOrEmpty(nota.ChaveNota))
            {
                MessageBox.Show("Nota Não possui Chave!", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            Selecionado.NotaReferencias.Add(new NotaReferencia 
            {
                Sequencia = Selecionado.NotaReferencias.Count ==0?1: 
                Selecionado.NotaReferencias.Max(n=> n.Sequencia) +1,
                Chave = nota.ChaveNota,
                Nota = Selecionado
            });

            AtualizarListaNfRef();
            lkpNotaRefLista.EditValue = null;
        }

        private void btnExcluiNfRef_Click(object sender, EventArgs e)
        {
            NotaReferencia nRef = (NotaReferencia)gvNfRef.GetFocusedRow();
            if(nRef == null)
            {
            ...
            var itemExcluir = Selecionado.NotaReferencias.Where(n => n.Sequencia == nRef.Sequencia).FirstOrDefault();
            if(itemExcluir != null)
            {
                ...
                Selecionado.NotaReferencias.Remove(itemExcluir);
                AtualizarListaNfRef();
            }
        }
    }
