...
namespace Aplicacao
{
    public partial class GridNotaGen : Aplicacao.IntermediariasTela.GridNotaIntermediaria
    {
        ...
        public static GridNotaGen Get(InOutType inOut, ProdutoServicoType produtoServico, bool notasCanceladas)
        {
            ...
            var modeloFiltro = inOut.Equals(InOutType.Saída) ? configuracao.NotaFiltroListagem : configuracao.ComprasFiltroListagem;
            ...
            parametros.Add("inOut", inOut);
            parametros.Add("produtoServico", produtoServico);
            parametros.Add("notasCanceladas", notasCanceladas);

            return new GridNotaGen(NotaController.Instancia.GetAllPeriodo(dataInicial, dataFinal, inOut, produtoServico, notasCanceladas), inOut, notasCanceladas, produtoServico, null, true, dataInicial, dataFinal, parametros, typeof(FormNota), inOut);
        }
        ...
        {
            return new GridNotaGen(NotaController.Instancia.GetAutorizadasPorCliente(inOut, produtoServico, notasCanceladas, idPessoa, situacao), inOut, notasCanceladas, produtoServico, null, false, null, null, null, typeof(FormNota), inOut);
        }

        private GridNotaGen(IList<Nota> lista, InOutType inOut, bool notasCanceladas, ProdutoServicoType produtoServico, Nota selecionado, bool adicionarFiltro, DateTime? dataInicial, DateTime? dataFinal, Dictionary<string, object> parametros, Type formManut, params object[] parametrosFormFilho)
            : base(lista, selecionado, adicionarFiltro, dataInicial, dataFinal, parametros, formManut, parametrosFormFilho)
        {
            this.inOut = inOut;
            ...
            ConfiguraBotaoCusto();
            ConfigurarPagamentosNF(); // ConfiguraBotaoCancelarNF();
        ...
        {
            base.OnShown(e);
            gvPrincipal.OptionsSelection.MultiSelect = false;
        ...
        private void ConfigurarPagamentosNF()
        {
            //sbFuncao23.Visible = false;
            //sbFuncao23.Enabled = false;
            sbFuncao23.Click += new EventHandler(btPagamentos_Click);
            sbFuncao23.Text = "&Pagamentos";
            //sbFuncao23.Image = global::Aplicacao.Properties.Resources.ImprimirCartaoPonto;
            sbFuncao23.Visible = true;
            sbFuncao23.Enabled = true;
            sbFuncao23.Size = new System.Drawing.Size(75, 23);
            sbFuncao23.Location = new Point(761, 426);
            //sbImprimirFiltro.Location = new Point(600, 426);
            //sbAlterar.Location = new Point(761, 399);
        }
        ...
        protected override void Duplicar()
        {
            if (gvPrincipal.GetRegistroSelecionado() != null)
            {
                ...
                registroSelecionado.CancDt = null;
                registroSelecionado.CancMotivo = null;
                registroSelecionado.CancUsuario = null;
                registroSelecionado.DtHoraCanc = null;
                registroSelecionado.Status = null;
        ...
        private void EnviarNFe()
        {
            ...
            if (registroSelecionado != null)
            {
                ...
                else
                {
                    ...
                    {
                        // Validação NFCE Data/Hora limite
                        if (registroSelecionado.ModeloDocto == 65
                            && (registroSelecionado.StatusLegivel == "Não enviada" || registroSelecionado.StatusLegivel == "Negada"))
                        {
                            var dataSelecionada = registroSelecionado.Dt;
                            var result = DateTime.Now - dataSelecionada;
                            if (result.TotalMinutes >= 5)
                            {
                                if (MessageBox.Show($"Deseja atualizar a hora para envio da NFC-e?\r\n\r\nData/Hora atual: {DateTime.Now}\r\nData/Hora informado: {dataSelecionada}", "Atualizar data NFCe", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                                {
                                    registroSelecionado.Dt = DateTime.Now;
                                    registroSelecionado.DtEnvio = DateTime.Now;
        ...
        private void ImpressaoNota()
        {
            ...
            if (notas != null && notas.Count > 0)
            {
                ...
                foreach (Nota nota in nfe)
                {
                    ...
                    nfeController.ImprimirNota(nota, true, NomeImpressora);
                }
        ...
        {
            var registroSelecionado = GetRegistroSelecionado();
            new FormNotaPagamentos(registroSelecionado).ShowDialog(this);
        }
        ...
        private void FormatarColunaStatus()
        {
            ...
            StyleFormatCondition estilo6 = new StyleFormatCondition();
            estilo6.Appearance.ForeColor = Color.Green;
            estilo6.Appearance.Options.UseForeColor = true;
            estilo6.ApplyToRow = false;
            estilo6.Column = gvPrincipal.Columns["StatusLegivel"];
            estilo6.Condition = FormatConditionEnum.Equal;
            estilo6.Value1 = "Importada";

            StyleFormatCondition estilo7 = new StyleFormatCondition();
            estilo7.Appearance.ForeColor = Color.Green;
            estilo7.Appearance.Options.UseForeColor = true;
            estilo7.ApplyToRow = false;
            estilo7.Column = gvPrincipal.Columns["StatusLegivel"];
            estilo7.Condition = FormatConditionEnum.Equal;
            estilo7.Value1 = "Importação Manual";
            ...
            gvPrincipal.FormatConditions.Add(estilo6);
            gvPrincipal.FormatConditions.Add(estilo7);
