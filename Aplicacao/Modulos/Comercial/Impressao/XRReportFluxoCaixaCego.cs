...
{
    public partial class XRReportFluxoCaixaCego : DevExpress.XtraReports.UI.XtraReport
    {
        public XRReportFluxoCaixaCego(FluxoCaixa Fluxo)
        {
            InitializeComponent();
            ...
            IList<pxFechamentoCaixaRateio> Itens = FechamentoCaixaRateioController.Instancia.GetRateiosPorFluxoDeCaixa(Fluxo.ID);
            foreach (var Item in Itens)
            {
                var DataRow = ovDS_Dados.Tables[0].NewRow();
                DataRow["DESCFORMAPAGAMENTO"] = Item.FormaPagamento;
                DataRow["TOTALPAGO"] = Item.ValorCaixa;
                DataRow["TOTALINFORMADO"] = Item.ValorInformado;
                DataRow["DIFERENCA"] = Item.ValorCaixa - Item.ValorInformado;
