...
{
    public partial class FormFinalizarVendaFrenteCaixaNovaPagamentos : MetroForm
    {
        ...
        private bool CreditoJaTotalizado;
        ...
        private decimal percentualDescontoValidado = 0;
        ...
        /* Novos Campos */
        private Condicao CondicaoConfigurada;
        private bool ModoDevolucao;

        public FormFinalizarVendaFrenteCaixaNovaPagamentos(FrenteCaixaNota frenteCaixaNota, FluxoCaixa _Fluxo, string _CPF, FormFrenteCaixaNota formFrenteCaixaNota, FormFrenteCaixaNotaNova formFrenteCaixaNotaNova,
            Pedido PedidoExistente = null, bool modoDevolucao = false)
        {
            InitializeComponent();

            ModoDevolucao = modoDevolucao;
            ...
            CondicaoConfigurada = frenteCaixaNota.Condicao ?? configuracao.CondicaoFrenteCaixa;
        ...
        private void MontarParcelas(Condicao CondicaoSelecioanda = null)
        {
            ...
            foreach (var Parcela in Parcelas)
            {
                ...
                var pxyFormaPgto = new pxyFormaPagamentoBaixaDoc
                {
                    ...
                    TipoDocumento = Parcela.TipoDocumento,
        ...
        {
            TipoNota tipoNota = TipoNotaController.Instancia.LoadObjectById(((TipoPedido)lkpTipoPedido.Selecionado).TipoNota.ID);
            if (tipoNota.Ent_Sai == 2)
            {
                ...
                try
                {
                    ...
                    if (false && ImeiController.VerificaUtilizacaoImei(objFrenteCaixaNota.ListaFrenteCaixaNotaItem.Select(s => s.Produto).ToList()))
                    {
                        ...
                        if (PedidoSelecionado == null)
                            objPedido = finalizador.GerePedido(objFrenteCaixaNota.TipoPedido.Ent_Sai, true);
                        else
                            objPedido = finalizador.GerePedidoComPedidoExistente(PedidoSelecionado, true);
        ...
        private void VerificarCreditoUtilizado()
        {
            if (!UtilizaCredito)
                return;

            bool usaCreditoNaForma = PossuiConfiguracaoDeFormaDePagamentoParaCredito();
            if (!usaCreditoNaForma)
                return;

            pxyFormaPagamentoBaixaDoc formaEncontrada = formasPagamento.Where(f => f.FormaPagamento.ID == configuracao.IDFormaPagamentoCreditoPDV.Value).FirstOrDefault();
            formaEncontrada.Valor += ValorCreditoUtilizado;
        }
        ...
        private bool PossuiConfiguracaoDeFormaDePagamentoParaCredito()
        {
            if (configuracao.IDFormaPagamentoCreditoPDV.HasValue && configuracao.IDFormaPagamentoCreditoPDV.Value != 0)
            {
                pxyFormaPagamentoBaixaDoc formaEncontrada = formasPagamento.Where(f => f.FormaPagamento.ID == configuracao.IDFormaPagamentoCreditoPDV.Value).FirstOrDefault();
        ...
        private void AtualizaCamposFrenteCaixa()
        {
            ...
            objFrenteCaixaNota.Condicao = CondicaoConfigurada;
        }
        ...
        {
            Troco = Convert.ToDecimal(txtTroco.EditValue);
            Close();
        }
        ...
        private void AtualizarGridFormasPagamento(bool mudarFoco = true)
        {
            ...
            {
                var Parcelar = ConversorFormaPagamento.GetFormaPagamento(Item.FormaPagamento.CodigoSefaz) == ParcelaBase.EnumFormaPagamento.CartaoCredito;
                flowLayoutPanel1.Controls.Add(new UC_PagamentoPDV(Item.FormaPagamento.ID, Item.FormaPagamento.Nome, Parcelar, Item.Valor,
                    5 + flowLayoutPanel1.Controls.Count, Item.Parcela, Item.FormaPagamento.Atalho));
            }

            (flowLayoutPanel1.Controls[flowLayoutPanel1.Controls.Count - 1] as UC_PagamentoPDV).TXT_Valor.Leave += TXT_Valor_Leave;
        ...
        private void ZerarPagamentos()
        {
            if (MostrarZeradoValorPagamentoFinalizarVenda)
            {
                ...
                foreach (UC_PagamentoPDV item in flowLayoutPanel1.Controls)
                {
        ...
        {
            if (percentualDesconto == percentualDescontoValidado)
                return true;

            if (percentualDesconto > configuracao.PercentualMaxDesconto)
            {
                var retorno = RequisitaGerente();
                if (retorno)
                    percentualDescontoValidado = percentualDesconto;
        ...
        {
            txtTotalAPagar.EditValue = objFrenteCaixaNota.TotalGeral - (CreditoJaTotalizado ? 0 : ValorCreditoUtilizado);
            CreditoJaTotalizado = true;
        }
        ...
        {
            lkpTipoPedido.EditValueChanged -= lkpTipoPedido_EditValueChanged;
            configuracao = configuracaoController.GetConfiguracao();
            lkpTipoPedido.EditValue = configuracao.TipoOrcamentoFrenteCaixa;
            if (PedidoSelecionado != null)
            ...
            if (ModoDevolucao)
            {
                ...
                if (tipoPedido != null)
                {
                    lkbTipoPedido.Enabled = false;
                    lkpTipoPedido.Enabled = false;
                    lkpTipoPedido.Localizar(tipoPedido.ID);
                }
            }
        }
        ...
        private bool ContemFocoNosComponentesDeValor()
        {
            foreach (Control control in flowLayoutPanel1.Controls)
                if ((control as UC_PagamentoPDV).TXT_Valor.ContainsFocus)
                    return true;
        ...
        public void EventoAtualizarFormaPagamento(int IDFormaPagamento, decimal ValorTotal, int QuantidadeParcelas, int Parcela)
        {
            var _forma = formasPagamento.FirstOrDefault(o => o.FormaPagamento?.ID == IDFormaPagamento && o.Parcela == Parcela);
            if (_forma != null)
                _forma.QuantidadeParcelas = QuantidadeParcelas;
            ...
            {
                formasPagamento.First().Valor = ValorTotal;
                AtualizarGridFormasPagamento();
                return;
            }

            var TotalAPagar = (objFrenteCaixaNota.TotalGeral - ValorCreditoUtilizado);
            var DiferencaDividir = TotalAPagar - Convert.ToDecimal(Arredondar(Convert.ToDouble(ValorTotal), 2));
        ...
        public void EventoDuploCliqueFormaPagamento(UC_PagamentoPDV control)
        {


            control.TXT_Valor.Focus();
            decimal.TryParse(txtFaltaPagar.EditValue.ToString(), out decimal valorPagar);
            if (valorPagar > 0)
            {
                var tXT_Valor = Convert.ToDecimal(control.TXT_Valor.EditValue.ToString().Replace(".", ","));
                var valorTotalAPagar = Convert.ToDecimal(txtTotalAPagar.EditValue.ToString());
                if (tXT_Valor > 0 && tXT_Valor != valorTotalAPagar)
                {
                    control.TXT_Valor.Text = (tXT_Valor + valorPagar).ToString();
                }
                else
                {
                    control.TXT_Valor.Text = valorPagar.ToString();
                }
            }

            control.TXT_Valor.SelectAll();
        }

        private void lkpTipoPedido_Leave(object sender, EventArgs e)
        {
            if (flowLayoutPanel1.Controls.Count > 0)
                (flowLayoutPanel1.Controls[0] as UC_PagamentoPDV).TXT_Valor.Focus();

            VerificarTipoPedido();
        }

        private void VerificarTipoPedido()
        {
            TipoPedido tipoPedido = (TipoPedido)lkpTipoPedido.Selecionado;
            if (tipoPedido == null)
                return;

            TipoNota tipoNota = TipoNotaController.Instancia.LoadObjectById(tipoPedido.TipoNota.ID);
            if (tipoNota.Ent_Sai == 1)
            {
                flowLayoutPanel1.Controls.Clear();
                txt_Mensagem.Text = "Operação de Devolução Selecionada";
                txt_Mensagem.Visible = true;
            }

            else
            {
                AtualizarGridFormasPagamento(false);
                txt_Mensagem.Text = "";
                txt_Mensagem.Visible = false;
            }
        }

        private void FormFinalizarVendaFrenteCaixaNovaPagamentos_KeyDown(object sender, KeyEventArgs e)
        {
            MandarFocoPagamento(e.KeyCode.ToString());
        }

        private void MandarFocoPagamento(string Atalho)
        {
            ...
            foreach (Control control in flowLayoutPanel1.Controls)
            {
                if ((control as UC_PagamentoPDV).Atalho.ToUpper().Equals(Atalho))
                {
                    (control as UC_PagamentoPDV).TXT_Valor.Focus();
                    decimal.TryParse(txtFaltaPagar.EditValue.ToString().Replace(".", ","), out decimal valorPagar);
                    if (valorPagar > 0)
                        (control as UC_PagamentoPDV).TXT_Valor.Text = valorPagar.ToString();

                    (control as UC_PagamentoPDV).TXT_Valor.SelectAll();
                }
            }
        }

        private void lkpTipoPedido_EditValueChanged(object sender, EventArgs e)
        {

        }

        private void lkpVendedor_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidaApenasNumeros(sender, e);
        }

        private void lkpTipoPedido_KeyPress(object sender, KeyPressEventArgs e)
        {
            ValidaApenasNumeros(sender, e);
        }

        private void ValidaApenasNumeros(object sender, KeyPressEventArgs e)
        {
        ...
        {
            decimal valorPagar = Convert.ToDecimal((string.IsNullOrEmpty(txtTotalAPagar.Text) ? "0" : ReplaceText(txtTotalAPagar.Text)));
            decimal valorProdutos = Convert.ToDecimal((string.IsNullOrEmpty(txtTotalProdutos.Text) ? "0" : ReplaceText(txtTotalProdutos.Text)));
            decimal valorDesconto = (valorProdutos - valorPagar) < 0 ? 0 : (valorProdutos - valorPagar);
            if (valorDesconto == 0)
            {
                txtTotalAPagar.EditValue = txtTotalProdutos.EditValue;
                txtValorDesconto.EditValue = 0;
                txtPercDesconto.EditValue = 0;
                objFrenteCaixaNota.TotalGeral = valorProdutos;
            }
            else
            {
                decimal perDesconto = Math.Round(((valorDesconto * 100) / valorProdutos), 4);
                if (PercentualDescontoPermitido(perDesconto))
                {
                    SetValorDesconto(valorDesconto);
                    SetPercentualDesconto(perDesconto);
                }
                else
                {
                    SetValorDesconto(0);
